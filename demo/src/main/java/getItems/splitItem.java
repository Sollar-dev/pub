...
public class splitItem {
    ...
    private String getProf(){
        Pattern pattern = Pattern.compile("(профессор)|(доцент)|(старший\\s*преподаватель)|(ассистент)|(начальник\\s+отдела\\s+(оп)\\s+и\\s+(учр))", Pattern.CASE_INSENSITIVE);
        Matcher matcher = pattern.matcher(item);
        if (matcher.find()){
            arr[4] = matcher.start();
            arr[5] = matcher.end();
